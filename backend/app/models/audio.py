...
class Audio(Base):
    ...
    id_cuenta_dueno: Mapped[str] = mapped_column(String(36), ForeignKey("cuentas.id"), nullable=False)
    ...
    lista_likes_cuentas: Mapped[list] = mapped_column(JSON, default=list)

    dueno: Mapped["Cuenta"] = relationship(back_populates="audios")
...
class Comentario(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    id_dueno_comentario: Mapped[str] = mapped_column(String(36), ForeignKey("cuentas.id"), nullable=False)
    id_audio: Mapped[str] = mapped_column(String(36), ForeignKey("audios.id"), nullable=False)
    texto: Mapped[str] = mapped_column(Text, nullable=False)
    num_likes: Mapped[int] = mapped_column(Integer, default=0)
    lista_likes_cuentas: Mapped[list] = mapped_column(JSON, default=list)

    dueno: Mapped["Cuenta"] = relationship(back_populates="comentarios")
